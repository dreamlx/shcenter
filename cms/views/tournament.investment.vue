...
<template>
    ...
    <!--        <Form ref="formInline" inline label-position="top">
        ...
        <Table  :columns="columns7" :data="investmentList"></Table>
...
export default {
    ...
    methods: {
        showDetail(index){
            this.$router.push({path:'/investment/update',query: {id: this.investmentList[index].id}});
           //this.createNews();
        },
        getList() {
            ...
            this.$http
                .post(
                    this.$root.$options.baseURI +
                        '/shcenter/api/sport/investment/list',params
                )
                .then(response => {
        ...
        createNews(){
            this.$router.push('investment/create');
        },
        doDel(){
            ...
            .get(
                this.$root.$options.baseURI +
                    '/shcenter/api/sport/investment/delete',{params : {id: this.investmentList[this.currentRowIndex].id} }
            )
            .then(response => {
                let res = response.data;
