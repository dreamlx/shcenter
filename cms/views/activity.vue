        ...
        <div style="padding:20px;">
            <Form  ref="formValidate" :model="formValidate" :rules="ruleValidate" >
                    ...
                    <FormItem  label="年份" prop="date">
                         <Select v-model="formValidate.date" placement='top-start'>
...
<script>
    ...
    export default {
        data () {
            const getDateList = ()=>{
                var d = new Date, start = 2017, end = d.getFullYear() + 3, dateList = [];
        ...
        methods: {
            ...
            getDetail(){
                ...
                .get(
                    this.$root.$options.baseURI +
                        '/shcenter/api/sport/activity/find',{params: {id: this.aid}}
                )
                .then(response => {
                    let res = response.data;
                    this.formValidate.title = res.activity.title;
                    this.formValidate.title_en = res.activity.title_en;
                    this.formValidate.banner = res.activity.banner;
                    this.formValidate.content = res.activity.content;
                    this.formValidate.content_en = res.activity.content_en;
                    this.formValidate.date = res.activity.date;
            ...
            doCreate(){
                ...
                this.$http.post('/shcenter/api/sport/activity/add',params)
                   .then(response => {
                       this.submiting = false;
                       this.$router.push({path:'/sport',query:{tab:'activities'}});
                   })
                .catch((response)=>{
                    this.submiting = false;
            ...
            doUpdate(){
                let params = Object.assign({id:this.aid},this.formValidate);
                this.submiting = true;
                this.$http.post('/shcenter/api/sport/activity/update',params)
                .then(function(res){
                    this.submiting = false;
