...
<template>
    <div class="parking-list-box">
        <div class="form-box">
           <Form ref="formInline" inline label-position="top">
                ...
                <FormItem  label="查询" class="ivu-form-btn-item">
                      ...
                      <Button type="primary"   :disabled='createDisable'  @click="createBanner">创建</Button>
                </FormItem>
         </Form>
        </div>
        <Table  :columns="columns7" :data="bannerList" ></Table>
       <!--  <div style="margin: 10px 0 10px;overflow: hidden;" v-if="list.length>0">
        ...
        <Modal title="View Banner" v-model="visible">
                        ...
                        render: (h, params) => {
                            return h('div', [
                                h(
                                    'Button',
                                    {
                                        ...
                                        on: {
                                            click: () => {
                                                this.visible = true;
                                                this.imgName = this.bannerList[params.index].src;
                                            }
                                        }
                                    },
                                    '预览'
                                ...
                                h('Button', {
                                    ...
                                    on: {
                                        click: () => {
                                            this.$router.push({path:'/banner/update',query: {id: this.bannerList[params.index].id}});
                                        }
                                    }
                                }, '编辑'),
                                h('Button', {
                ...
                this.$http
                    .post(
                        this.$root.$options.baseURI +
                            '/shcenter/api/sport/banner/list',{...params}
                    )
                    .then(response => {
